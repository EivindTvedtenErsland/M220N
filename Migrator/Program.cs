...
namespace Migrator
{
    ...
    class Program
    {
        ...
        static string mongoConnectionString = "mongodb+srv://m220student:m220password@mflix.anwaq.mongodb.net/mflix?retryWrites=true&w=majority";
        ...
        static async Task Main(string[] args)
        {
            ...
            if (datePipelineResults.Count > 0)
            {
                ...
                foreach (var element in datePipelineResults)
                {
                    listWrites.Add(new ReplaceOneModel<Movie>(Builders<Movie>.Filter.Eq(el => el.Id, element.Id), element));
                }
            ...
            if (ratingPipelineResults.Count > 0)
            {
                ...
                var listWrites = new List<WriteModel<Movie>>();
                foreach (var element in ratingPipelineResults)
                {
                    listWrites.Add(new ReplaceOneModel<Movie>(Builders<Movie>.Filter.Eq(el => el.Id, element.Id), element));
                }
