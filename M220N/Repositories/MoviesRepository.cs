...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        {
            // Probably better to add the writeconcern to the mongoURI, since now it only applies to the MoviesRepository _mongoClient
            _mongoClient = mongoClient.WithWriteConcern(writeConcern: new WriteConcern(wTimeout: TimeSpan.FromMilliseconds(2500)));
        ...
        public async Task<Movie> GetMovieAsync(string movieId, CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (ex.Message.Contains("EE11000"))
                {
                    Console.WriteLine(ex.Message);
                    return null;
                }
                else if (ex.Message.Contains("valid 24 digit hex string"))
                {
                    Console.WriteLine(ex.Message);
                    return null;
                }
                else if (ex.Message.Contains("invalid characters"))
                {
                    Console.WriteLine(ex.Message);
                    return null;
                }
            ...
            int page = 0, params string[] genres)
        {
            ...
            returnValue = await _moviesCollection.Find<Movie>(filter)
                                                    .Limit(limit)
                                                    .Skip(limit * page)
                                                    .Sort(sort)
                                                    .ToListAsync(cancellationToken);
