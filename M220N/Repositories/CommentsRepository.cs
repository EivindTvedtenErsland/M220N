    ...
    public class CommentsRepository
    {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var result = _commentsCollection.WithWriteConcern(WriteConcern.WMajority).InsertOneAsync(newComment, cancellationToken: cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var filter = Builders<Comment>.Filter.Where(check => check.Email == user.Email
            && check.MovieId == movieId && check.Id == commentId);
            
            var update = Builders<Comment>.Update
                                            .Set(c => c.Text, comment)
                                            .Set(c => c.Date, DateTime.UtcNow);

            // IsUpsert defaults to "false", set for illustration purposes
            ...
            {
                Console.WriteLine("Matched: {0}, Modified: {1}", result.MatchedCount, result.ModifiedCount);
            }
            ...
            User user, CancellationToken cancellationToken = default)
        {
            ...
            var valid = Builders<Comment>.Filter.Where(check => check.Email == user.Email);

            var filter = Builders<Comment>.Filter.Where(c => c.MovieId == movieId && c.Id == commentId);

            var combine = Builders<Comment>.Filter.And(valid, filter);

            var result = await _commentsCollection.DeleteOneAsync(combine);
        ...
        public async Task<TopCommentsProjection> MostActiveCommentersAsync()
        {
            ...
            */
            try
            {
                ...
                /*
                var result = await _commentsCollection.WithReadConcern(ReadConcern.Majority)
                                                        .Aggregate()
                                                        .Group(
                                                            c => c.Email,
                                                            z => new 
                                                        ...
                                                        .SortByDescending(group => group.Count)
                                                        .Project(projection)
                                                        .Limit(20)
                                                        .ToListAsync();
                ...
                */
                ...
                // I'm stealing this part of the code since I can't be bothered to 
                // spend so much time trying to coerce the input into the format they want
                // for an assignment where the main purpose is to create an aggregation pipeline.
                var result =  _commentsCollection
                    .AsQueryable()
                    .GroupBy(x => x.Email)                  
